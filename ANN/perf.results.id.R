...
## clear memory and set large memory limit for virtual machine (~64GB)
# rm(list=ls())
# gc()
...
#### perf.results.id - function to provide calibration and performance tables, metrics, and plots
####
perf.results.id <- function(pred, # predicted probability of outcome=1 for each observation in the test set
  ...
  names(calib.tab) <- c("Pctile","AvgPredRisk","ObsRisk","Vists","Events","UniqueEvents")
  ...
  perf.tab <- rbindlist(lapply(train.pctile, function(x){tail(tab.rocr[RiskCutpoint>=x], 1)}))
